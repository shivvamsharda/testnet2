...
function addMediaMetricsToStore(store, onError) {
    ...
    if (shouldOpenNewSocket) {
      ...
      try {
        const createNewWebSocket = async (numRetries = 0) => {
          const reportingWebsocketUrl = await controller.getMetricsReportingUrl(playbackId, currentPlaybackUrl, store.getState().sessionToken);
  ...
  try {
    const destroyTtffListener = store.subscribe((state, prevState) => {
      ...
      if (state.playbackError?.type !== prevState.playbackError?.type && state.playbackError) {
