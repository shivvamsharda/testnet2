...
class BaseLivepeerProvider {
  ...
  async _get(url, options) {
    const response = await fetch__default["default"](`${this._config.baseUrl}${url}`, {
      method: 'GET',
      ...options,
      headers: {
        ...options?.headers,
        'x-core-sdk-version': version.version.core,
        'x-react-sdk-version': version.version.react,
        'x-react-native-sdk-version': version.version.reactNative
      }
    });
    if (!response.ok) {
