...
var useInternalQuery = require('../../dist/useInternalQuery-34f85f2d.cjs.dev.js');
...
const useBroadcast = ({
  ...
  const provider = system.useLivepeerProvider();
  const ingestUrl = React__namespace.useMemo(() => {
  ...
  const [broadcastError, setBroadcastError] = React__namespace.useState(null);
  const onBroadcastError = React__namespace.useCallback(error => {
    const newError = error ? {
      type: core.isAccessControlError(error) ? 'access-control' : core.isStreamOfflineError(error) ? 'offline' : 'unknown',
  ...
  const broadcastRef = React__namespace.useCallback(element => {
    if (element) {
      setMediaElement(element);
      mediaElementRef?.(element);
    }
  }, [mediaElementRef]);
  const broadcastProps = React__namespace.useMemo(() => ({
    ref: broadcastRef,
    ingestUrl,
    objectFit: objectFit,
    options: controls,
  ...
  const mediaControllerProps = React__namespace.useMemo(() => ({
    autoPlay: false,
  ...
  const controlsContainerProps = React__namespace.useMemo(() => ({
...
const useClipButton = props => {
  ...
  const title = React__namespace.useMemo(() => `Clip last ${Number(length).toFixed(0)}s (x)`, [length]);
  const isShown = React__namespace.useMemo(() => playbackId && live, [playbackId, live]);
  return {
    title,
    isShown: isShown,
    status: clipStatus,
...
const useSourceMimeTyped = ({
  ...
  const combinedPlaybackInfo = React__namespace.useMemo(() => playbackInfo ?? resolvedPlaybackInfo, [playbackInfo, resolvedPlaybackInfo]);
  const fetchAccessKey = React__namespace.useCallback(async () => {
    if (accessKey) {
      return accessKey;
    }
  ...
  const [type, sourceMimeTyped] = React__namespace.useMemo(() => {
    ...
    const mediaSourceTypes = sources.map(s => typeof s === 'string' ? core.getMediaSourceType(s) : null).filter(s => s);
  ...
  const sourceMimeTypedSorted = React__namespace.useMemo(() => {
...
const usePlayer = ({
  ...
  const [mediaElement, setMediaElement] = React__namespace.useState(null);
  const sessionToken = React__namespace.useMemo(() => generateRandomToken(), [playbackId]);
  const {
    source,
    uploadStatus,
    jwtResolved,
  ...
  const [playbackError, setPlaybackError] = React__namespace.useState(null);
  const onPlaybackError = React__namespace.useCallback(error => {
    const newPlaybackError = error ? {
      type: core.isAccessControlError(error) ? 'access-control' : core.isBframesError(error) || core.isNotAcceptableError(error) ? 'fallback' : core.isStreamOfflineError(error) ? 'offline' : 'unknown',
      message: error?.message ?? 'Error with playback.'
    } : null;
  ...
  const [hasBeenShown, setHasBeenShown] = React__namespace.useState(false);
  const loaded = React__namespace.useMemo(() => priority || _isCurrentlyShown || hasBeenShown, [priority, _isCurrentlyShown, hasBeenShown]);
  React__namespace.useEffect(() => {
  ...
  const playerProps = React__namespace.useMemo(() => ({
    ref: playerRef,
    autoPlay,
    playbackId,
    muted,
  ...
  const sourceWithLoaded = React__namespace.useMemo(() => loaded ? source : null, [loaded, source]);
  return {
    mediaElement,
...
exports.useClipButton = useClipButton;
